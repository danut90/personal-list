module.exports = (db) => {
	...
	return {
		findAll: (req, res) => {
			db.query('SELECT l.*, concat(u.first_name, \' \', u.last_name) AS "userName" ' +
			'FROM "UserError" l ' +
		...
		destroy: (req, res) => {
			db.query('DELETE FROM "UserError" WHERE id = ' + req.params.id).then(resp => {
				if (resp[1].rowCount > 0) {
					...
					saveError(req.user, 'destroy UserError at id: ' + req.params.id, 'not found', res);
				}
			}).catch(e => saveError(req.user, 'destroy UserError at id: ' + req.params.id, e, res));
		},
